...
// Function to exclude array of days from recurring events
...
// Export 'Reference Cards' from JMC to 'schedules/students.csv'
fs.readFile(__dirname + '/schedules/students.csv', function (err, data) {
  if (err) {
    throw err; 
  }
		...
		async.each(output, function (line) {
			...
			if (!calendars[studentId]) {
				...
				noSchool.dates.forEach(function(day){
					event = calendars[studentId].createEvent({
						timestamp: new Date(),
						summary: day.label,
			...
			if (courseName) {
				if (periods[period] && year[term]) {
					...
					console.log('invalid period or term, student ' + studentId);
				}
			} else {
