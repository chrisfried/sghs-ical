...
var ICalEvent = function(_data) {
	...
	this.repeating = function(repeating) {
		...
		if(repeating.exdate) {
			if(!Array.isArray(repeating.exdate)){
				throw '`repeating.exdate` must be an Array of Dates!';
			}
			
			data.repeating.exdate = repeating.exdate;
		}
		
		if(!repeating.exrule) {
			return data.repeating.exrule;
		}
		
		if(repeating.exrule) {
			if(repeating.exrule && (!repeating.exrule.freq || vars.allowedRepeatingFreq.indexOf(repeating.exrule.freq.toUpperCase()) === -1)) {
				throw '`repeating.exrule.freq` is a mandatory item, and must be one of the following: ' + vars.allowedRepeatingFreq.join(', ') + '!';
			}
			data.repeating.exrule = {
				freq: repeating.exrule.freq.toUpperCase()
			};
		}

		if(repeating.exrule.count) {
			if(!isFinite(repeating.exrule.count)) {
				throw '`repeating.exrule.count` must be a Number!';
			}

			data.repeating.exrule.count = repeating.exrule.count;
		}

		if(repeating.exrule.interval) {
			if(!isFinite(repeating.exrule.interval)) {
				throw '`repeating.exrule.interval` must be a Number!';
			}

			data.repeating.exrule.interval = repeating.exrule.interval;
		}

		if(repeating.exrule.until) {
			if(!(repeating.exrule.until instanceof Date)) {
				throw '`repeating.exrule.until` must be a Date Object!';
			}

			data.repeating.exrule.until = repeating.exrule.until;
		}
		
		if(repeating.exrule.byday) {
			if(!Array.isArray(repeating.exrule.byday)){
				throw '`repeating.exrule.byday` must be an Array of day symbols!';
			}
			
			data.repeating.exrule.byday = repeating.exrule.byday;
		}
	...
	this.generate = function(calendar) {
		...
		if(data.repeating) {
			...
			if(data.repeating.exdate) {
				var formatedDates = new Array();
				data.repeating.exdate.forEach(function(derp) {
					formatedDates.push(tools.formatDate(derp));
				});
				g += 'EXDATE:' + formatedDates.join(',') + '\r\n'; 
			}
			
			if(data.repeating.exrule) {
				g += 'EXRULE:FREQ=' + data.repeating.exrule.freq;

				if(data.repeating.exrule.count) {
					g += ';COUNT=' + data.repeating.exrule.count;
				}
	
				if(data.repeating.exrule.interval) {
					g += ';INTERVAL=' + data.repeating.exrule.interval;
				}
	
				if(data.repeating.exrule.until) {
					g += ';UNTIL=' + tools.formatDate(data.repeating.exrule.until);
				}
	
				if(data.repeating.exrule.byday) {
					g += ';BYDAY=' + data.repeating.exrule.byday.join(',');
				}
